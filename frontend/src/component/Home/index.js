...
const Home = () => {
  ...
  const fetchTodos = async () => {
    ...
    try {
      const response = await fetch(
        "https://todo-application-backend-rk3h.onrender.com/",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        data.length === 0
          ? setApiStatus(apiConstrains.noData)
          : setApiStatus(apiConstrains.onSuccess);
      } else {
  ...
  const onChangeStatus = async (id, status) => {
    ...
    const response = await fetch(
      "https://todo-application-backend-rk3h.onrender.com/change-status",
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
  ...
  const onClickAddBtn = async () => {
    if (todoText.trim() !== "") {
      ...
      try {
        const response = await fetch(
          "https://todo-application-backend-rk3h.onrender.com/add-todo",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const onClickDeleteBtn = async (id) => {
    ...
    const response = await fetch(
      "https://todo-application-backend-rk3h.onrender.com/delete-todo ",
      {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
    ...
    switch (apiStatus) {
      ...
      case apiConstrains.noData:
        return (
          ...
          >
            You have no tasks yet! <br /> Start adding your{" "}
            <span style={{ color: "black", fontSize: "20px" }}>to-dos</span>{" "}
            and stay on track.
          </p>
        );
      ...
      <div className="todos-bg-container">
        <div className="main-card">
          <h1 className="create-task-heading">
            Schedule <span className="create-task-heading-subpart">Task</span>
          </h1>
          <input
            type="text"
            value={todoText}
